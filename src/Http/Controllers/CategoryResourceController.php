...
use Form;
use Litecms\Faq\Http\Requests\CategoryRequest;
...
class CategoryResourceController extends BaseController
{
    ...
    public function __construct(CategoryRepositoryInterface $category)
    {
        ...
        $this->repository
            ...
            ->pushCriteria(\Litecms\Faq\Repositories\Criteria\CategoryResourceCriteria::class);
    }
    ...
    public function index(CategoryRequest $request)
    {
        ...
        if ($this->response->typeIs('json')) {
            ...
            return $this->repository
                ->setPresenter(\Litecms\Faq\Repositories\Presenter\CategoryPresenter::class)
                ->$function();
        }
    ...
    public function store(CategoryRequest $request)
    {
        try {
            ...
            return $this->response->message(trans('messages.success.created', ['Module' => trans('faq::category.name')]))
                ->code(204)
                ->status('success')
                ->url(guard_url('faq/category/' . $category->getRouteKey()))
                ->redirect();
        } catch (Exception $e) {
            return $this->response->message($e->getMessage())
                ->code(400)
                ->status('error')
                ->url(guard_url('/faq/category'))
                ->redirect();
        }
    ...
    public function destroy(CategoryRequest $request, Category $category)
    {
        try {
            ...
            $category->delete();
            return $this->response->message(trans('messages.success.deleted', ['Module' => trans('faq::category.name')]))
                ->code(202)
                ->status('success')
                ->url(guard_url('faq/category/0'))
                ->redirect();
    ...
    public function restore(CategoryRequest $request)
    {
        try {
            ...
            return $this->response->message(trans('messages.success.restore', ['Module' => trans('faq::category.name')]))
                ->status("success")
                ->code(202)
                ->url(guard_url('/faq/category'))
                ->redirect();
